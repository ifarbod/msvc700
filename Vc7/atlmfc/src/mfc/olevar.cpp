...
CArchive& AFXAPI operator>>(CArchive& ar, CComBSTR& string)
{
	...
	if( length > 0 )
	{
		...
		ar.Read(temp, length*sizeof(OLECHAR));
		string.Attach( temp );
	}
	else
		string.Empty();
...
void AFXAPI DDX_Text(CDataExchange* pDX, int nIDC, GUID& value)
{
   ...
   pDX->m_pDlgWnd->GetDlgItem(nIDC, &hWndCtrl);
	if (pDX->m_bSaveAndValidate)
	{
		...
		pszGUID = T2COLE(LPCTSTR(strTemp));
		hResult = CLSIDFromString(LPOLESTR(pszGUID), &value);
		if (FAILED(hResult))
		{
	...
	else
	{
		...
		AfxSetWindowText(hWndCtrl, OLE2CT(szGUID));
	}
}
